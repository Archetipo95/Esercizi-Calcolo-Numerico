function [] = risultati_bis(a,b,f,toll,maxit,xv,slv,fxv) 
...
%iterazioni necessarie per acuratezza T=epsilon/2
T=toll/2;
acc = ceil(log2((b-a)/T)-1);
disp('iterazioni necessarie: ');
disp(acc);

%controllo itfinale != acc && itfinale == itmax
if(n==maxit && n~=acc)
    disp('Attenzione indice max raggiunto e accuratezza no');
end